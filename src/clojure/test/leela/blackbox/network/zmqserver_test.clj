...
(deftest test-zmqserver-handle-message
  (storage/with-session [cluster ["127.0.0.1"] "leela"]
    ...
    ;;   (is (= (server/msg-label1 nil) (server/handle-message cluster ["get" "label" "ext" "0x03" "foobar"]))))

    ;; (testing "getlabel-exact after putlabel"
    ;;   (server/handle-message cluster ["put" "label" "0x03" "foobar"])
    ;;   (is (= (server/msg-label1 "foobar") (server/handle-message cluster ["get" "label" "ext" "0x03" "foobar"]))))
    ...
    ;;     (is (= (server/msg-link ["0x02"]) (server/handle-message cluster ["get" "link" "0x00" "0x01"])))
    ;;     (is (= (server/msg-link ["0x03"]) (server/handle-message cluster ["get" "link" "0x00" "0x02"])))
    ;;     (is (= (server/msg-link []) (server/handle-message cluster ["get" "link" "0x00" "0x03"])))))))
