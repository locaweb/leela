...
etcprefix = (["%s" % prefix, "%s/.." % prefix]
             [os.path.isdir("%s/../etc" % prefix)])
...
setup(name='leela-server',
      ...
      data_files=[('%s/sbin' % prefix, ['src/sbin/leela-server',
                  ...
                  ('%s/share/leela/templates' % prefix,
                   ['src/share/templates/line-irregular.tmpl']),
                  ('%s/share/leela/static/js' % prefix,
                   ['src/share/static/js/adapters/prototype-adapter.js',
